...
const LABELS = {
  en: ['What happened', 'Why', 'Next', 'Details', 'No reason code.'],
  th: ['เกิดอะไรขึ้น', 'ทำไม', 'ควรทำอะไรต่อ', 'รายละเอียดทางเทคนิค', 'ไม่มี reason code'],
};
...
export default function HumanDecisionExplanation({ codes, language = 'th', stage = null, compact = false, testId }) {
  ...
  if (compact) {
    ...
    return <div className="human-explanation-compact" data-testid={testId}><strong>{primary.title}</strong><span>{primary.explanation}</span><details><summary>{technical}</summary><div className="human-technical-codes">{explanations.map((item) => <code key={item.code}>{item.code}</code>)}</div></details></div>;
  }
  return (
    <div className="human-explanation-stack" data-testid={testId}>
      {explanations.map((item, index) => (
        <article className={`human-explanation-card${item.known ? '' : ' unknown'}`} key={item.code}>
          ...
          <details className="human-technical-details"><summary>{technical}</summary><div><code>{item.code}</code><span>{stage || 'unknown'}</span></div></details>
        </article>
      ))}
    </div>
  );
