...
const LABELS = {
  en: ['What happened', 'Why', 'What to do next', 'Technical details', 'Source reason codes from Manager_Agent', 'No reason code was supplied for this candidate.'],
  th: ['เกิดอะไรขึ้น', 'ทำไม', 'ควรทำอะไรต่อ', 'รายละเอียดทางเทคนิค', 'Reason code ต้นทางจาก Manager_Agent', 'Manager_Agent ไม่ได้ส่ง reason code สำหรับ Candidate นี้'],
};
...
export default function HumanDecisionExplanation({ codes, language = 'th', stage = null, compact = false, testId = 'human-decision-explanation' }) {
  const [what, why, next, technical, technicalHint, noReason] = LABELS[language === 'en' ? 'en' : 'th'];
  ...
  if (compact) {
    ...
    return (
      <div className="human-explanation-compact" data-testid={testId}>
        ...
        <details><summary>{technical}</summary><div className="human-technical-codes"><span>{technicalHint}</span>{explanations.map((item) => <code key={item.code}>{item.code}</code>)}</div></details>
      </div>
    );
  }
    ...
    <div className="human-explanation-stack" data-testid={testId}>
      {explanations.map((item, index) => (
        <article className={`human-explanation-card${item.known ? '' : ' unknown'}`} key={item.code}>
          ...
          <details className="human-technical-details"><summary>{technical}</summary><div><span>{technicalHint}</span><code>{item.code}</code><span>stage: {stage || 'unknown'}</span></div></details>
        </article>
      ))}
    </div>
  );
